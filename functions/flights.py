...
def load_flights(file_path_flights, file_path_airplanes):
    ...
    for row in data:
        idFlight, idAirplane, origin, destination, distance, duration, ticketPrice, points = row
        airplane = next((a for a in airplanes if a.id == int(idAirplane)),None)
        flights.append(Flight(int(idFlight), airplane, origin, destination, float(distance), float(duration), float(ticketPrice), int(points.strip())))
    return flights